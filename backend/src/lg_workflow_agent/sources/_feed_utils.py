"""Shared utilities for RSS/feed-based sources."""
...
def relevance_score(
    ...
    """Score how relevant an item is to the search topic (0.0 – high float)."""
...
async def fetch_feed(client: httpx.AsyncClient, name: str, url: str) -> tuple[str, str]:
    """Fetch a feed URL and return (name, content) or (name, '') on failure."""
    try:
