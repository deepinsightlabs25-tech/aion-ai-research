...
def create_node_classifier(llm, db):
    ...
    # Simple query classification cache (Phase 2 optimization)
    QUERY_TYPE_CACHE = {
        "trends": "summary",
        "latest": "summary",
        "news": "summary",
...
def create_node_task_generator(llm, db):
    ...
    def node_task_generator(state: WorkflowState):
        ...
        # Phase 3 optimization: Smart sub-agent selection based on query complexity
        # Production-safe: Only optimize for genuinely simple queries
        query_word_count = len(query.split())

        # Very short queries (< 5 words) - use minimal agents (saves 10-15s)
        # Production guard: Only for summary/blog type queries
        ...
        # Production guard: Only for summary queries
...
def create_node_validator(llm, db):
    ...
    def node_validator(state: WorkflowState):
        ...
        # Phase 2 optimization: Skip validation for simple queries (saves 5-10s)
...
def create_node_report_finalizer(llm, db):
    ...
    def node_report_finalizer(state: WorkflowState):
        ...
        # Phase 3 optimization: Skip chart generation for very short/simple reports (saves 2-5s)
        # Production-safe: Only skip if report is genuinely too short for meaningful visualizations
